...
export default function Home() {
  const tools = [
    { title: 'Images to PDF', desc: 'The best way to convert photos to professional PDF documents with advanced reordering and layout controls.', icon: ImageIcon, href: '/tools/images-to-pdf', category: 'Main Tool' },
    { title: 'AI PDF Summary', desc: 'Get instant insights and intelligent summaries from any document with EasyFiles AI.', icon: Sparkles, href: '/tools/ai-pdf-summary', category: 'AI' },
    { title: 'Compress PDF', desc: 'Reduce PDF file size intelligently without losing visual quality for easy emailing.', icon: Clock, href: '/tools/compress-pdf', category: 'Optimization' },
    { title: 'Merge PDF', desc: 'Combine multiple files into one professional document in the exact order you need.', icon: FileText, href: '/tools/merge-pdf', category: 'PDF' },
    { title: 'AI Scanner', desc: 'High-accuracy OCR and AI enhancement to turn your document photos into searchable PDFs.', icon: Cpu, href: '/tools/ai-scanner', category: 'AI' },
    { title: 'Secure Split', desc: 'Extract specific pages or page ranges safely with our precision splitting tool.', icon: Shield, href: '/tools/split-pdf', category: 'PDF' },
  ];
            ...
            <Link key={tool.title} href={tool.href}>
              <Card className="glass tool-card p-8 border-none flex flex-col h-full group hover:border-primary/20">
                ...
                <div className="flex items-center text-sm font-bold text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                  Try {tool.title} <ArrowRight className="ml-2 h-4 w-4" />
                </div>
              </Card>
            </Link>
          ))}
